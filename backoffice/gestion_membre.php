...
<table border="1" class="table table-striped table-bordered table-hover">
	...
	<?php foreach ($membre as $indice => $valeur): ?>
		<tr>
			...
			<td><?php $commande = haveCommandeByMembre($valeur['id_membre']); ?><a href="#" onClick="<?php if($commande): ?>InfoMessage()<?php else: ?>ConfirmSuppr()<?php endif; ?>"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>
			<script type="text/javascript">
				function ConfirmSuppr() {
					if (confirm("Voulez-vous supprimer ce membre ? Cela entrainera la suppression des avis qu'il a rédigé.")) { // Clic sur OK
						document.location.href="supprimer_membre.php?id=<?= $valeur['id_membre']; ?>";
					}
				}
